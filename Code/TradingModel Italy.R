...
for(i in 50:3532)
{
  ...
  fit <- lm(train$IT_var ~ train$Lag1_Var_ISP + train$Lag2_Var_ISP+
                               train$Lag1_Var_BMPS + train$Lag2_Var_BMPS +
                               train$Lag1_Var_UCG + train$Lag2_Var_UCG +
                               train$Lag1_Var_BPE + train$Lag2_Var_BPE +
                               train$Lag1_Var_MB + train$Lag2_Var_MB +
                               train$Lag1_Var_PMI + train$Lag2_Var_PMI)

  #runs the model and computes the errors
  prediction <- summary(fit)$coefficients[1] + summary(fit)$coefficients[2] * Banks_quotes$Lag1_Var_ISP[i+1] +
    summary(fit)$coefficients[3] *Banks_quotes$Lag2_Var_ISP[i+1] +
    summary(fit)$coefficients[4] *Banks_quotes$Lag1_Var_BMPS[i+1] +
    summary(fit)$coefficients[5] *Banks_quotes$Lag2_Var_BMPS[i+1] +
    summary(fit)$coefficients[6] *Banks_quotes$Lag1_Var_UCG[i+1] +
    summary(fit)$coefficients[7] *Banks_quotes$Lag2_Var_UCG[i+1] +
    summary(fit)$coefficients[6] *Banks_quotes$Lag1_Var_BPE[i+1] +
    summary(fit)$coefficients[7] *Banks_quotes$Lag2_Var_BPE[i+1] +
    summary(fit)$coefficients[6] *Banks_quotes$Lag1_Var_MB[i+1] +
    summary(fit)$coefficients[7] *Banks_quotes$Lag2_Var_MB[i+1] +
    summary(fit)$coefficients[6] *Banks_quotes$Lag1_Var_PMI[i+1] +
    summary(fit)$coefficients[7] *Banks_quotes$Lag2_Var_PMI[i+1]


  actual <- Banks_quotes$IT_var[i + 1]
  long_spread <- (Banks_quotes$DE_price[i + 1]/Banks_quotes$DE_price[i]-1) - (Banks_quotes$IT_price[i + 1]/Banks_quotes$IT_price[i]-1)
...
write.csv(output, "chartsdata.csv")
